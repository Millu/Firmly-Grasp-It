...
This document proposes how to successfully navigate a DE2 robot around a serious of 12 points in a minute using algorithms and the sensors available on the robot. This will be accomplished in steps. The first will preprocess the list of points and output an ideal order for the points to be naviagated to in. The first solution will be to visit the points in a random order. Next the greedy algorithm will be implemented, where the robot will visit the point immediately after it. The final opitimized solution will consist of the robot moving in smooth arcs to be most effcient in travelling to the points.
...
Our main plain to try and navigate to all of the points successfully using smooth arcs as described in the Braitenberg vehicles report. However, this may not be feasible due to the lack of predictability in turning between robots on a day to day basis. This also may not be feasible as the robot will have to compute the angles and keep track of its current angle relative to the next point. If this does not work, we will implement a navigation method where robot will turn only at 90 degree angles, resulting in movement in a grid-like pattern to navigate to all of the points.
